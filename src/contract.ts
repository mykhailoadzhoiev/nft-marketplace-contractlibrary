...
export class ContractsWrapper {
  ...
  createNewNFT = async (to: string, amountOfCopies: number = 1): Promise<number[]> => {
    const receipt = await this.contracts.nftFactory.methods.mint(amountOfCopies, to).send({ from: this.contracts.web3.eth.defaultAccount, gas: 2000000 })
  ...
  getOwnedItems = async (contractAddress: string, ownerAddress: string): Promise<{ id: string; metadata: string | undefined; }[]> => {
  ...
  createSellerSignature = async (params: SellerSignatureData) => {
    ...
    const sellerData = this.contracts.web3.utils.soliditySha3(
      orderMaker, isFixedPrice, price, itemId, itemContractAddress
    )
  ...
  createBuyerSignature = async (params: BuyerSignatureData) => {
    ...
    const buyerData = this.contracts.web3.utils.soliditySha3(
      orderTaker, isFixedPrice, price, extra, itemId, itemContractAddress
    )
  ...
  atomicMatch = async (params: AtomicMatchParams) => {
    ...
    return await this.contracts.nftMarketplace.methods.atomicMatch(order, params.buyerSignature, params.sellerSignature).send({ from: this.contracts.web3.eth.defaultAccount, gas: 2000000 })
  }
